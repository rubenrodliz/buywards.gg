@props(['titulo', 'class', 'rank', 'tier', 'lp', 'wins', 'losses', 'winrate'])
...
<div class="flex flex-col text-text_light border-2 border-primary border-solid rounded-lg p-4 bg-transparent h-[130px] bg-opacity-60 bg-bg_dark {{ $class }}">
    ...
    <div class="flex flex-row flex-wrap justify-center items-center">
        <img src="{{ asset('images/ranks/' . strtolower($rank)  . '.webp') }}" alt="league rank" class="w-[20%] mr-3">
        <div class="flex-1 flex flex-col">
            <div class="text-xl font-semibold text-{{ strtolower($rank) }}">
                {{ $rank }} {{ $tier }}
            </div>
            <div>{{ $lp }} LP</div>
        </div>
